...
void read_DataLine(Config &config,std::string line,std::map<std::string,std::string>& keymap,size_t atomindex,bool noproperties){
    ...
    std::vector<std::string> tokens;
    float token;
    std::cout << line << std::endl; //第一次

    if (noproperties){
        std::cout << line << std::endl; //第二次
...
Config read_xyz(const std::string filename){
    ...
    if (Linetype == State::FirstLine) {
        ...
        std::cout << "Start reading data line......" <<std::endl;
        for (size_t atomindex = 0; atomindex < config.pos_x.size(); atomindex++) {
            bool flag = judge_noproperties(keymap);
